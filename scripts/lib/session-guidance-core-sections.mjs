...
export function buildSessionModelLines(sessionContext = {}) {
  ...
  lines.push('- hello2cc 会把当前会话固定在 “Opus-compatible 的 Claude Code 原生操作方式”：优先宿主真实暴露的工具、agent、skills / workflows、MCP / connected tools 与计划任务习惯，而不是退回第三方模型自己的默认风格。');
...
export function buildWorkingHabitLines() {
  return [
    ...
    '- 比较 / 选型 / 能力边界问题默认直接回答；当表格更清晰时，优先“一句话判断 + 紧凑对比表 + 建议”，不要先绕去 plan。',
...
export function buildToolSearchLines() {
  return [
    ...
    '- hello2cc 不会把第三方模型带回“模型原生私有工作流”；它只把你固定在 Claude Code 真正暴露的能力面上。',
  ];
}
